...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* FULL SCREEN MENU */}
      <div className={`fixed z-20 inset-0 bg-black text-white flex flex-col items-center justify-center gap-8 transition-transform duration-500 ${menuOpen ? "translate-y-0" : "-translate-y-full"}`} onClick= {(()=> setMenuOpen(false))} >
        <Link to="/" className='menu-link'>Home</Link>
        <Link to="projects" className='menu-link'>Projetos</Link>
        <a href="mailto: witalonlopez@gmail.com" className='menu-link'>Contato</a>
      </div>
    </>
  )
}
